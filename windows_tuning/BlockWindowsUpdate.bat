...
rem https://www.windowscentral.com/how-stop-updates-installing-automatigotoy-windows-10
...
echo %WU_KEY%
"%reg%" ADD %WU_KEY% /f || goto :exitWithError

echo %AU_KEY%
"%reg%" ADD %AU_KEY% /f || goto :exitWithError
...
%write% :AUOptions = 5: 
"%reg%" ADD %AU_KEY% /v AUOptions /t REG_DWORD /d 5 /f || goto :exitWithError

echo. && echo disabling Automatic Updates -
%write% :NoAutoUpdate = 1: 
"%reg%" ADD %AU_KEY% /v NoAutoUpdate /t REG_DWORD /d 1 /f || goto :exitWithError

echo. && echo the computer gets updates from a WSUS server -
%write% :UseWUServer = 1: 
"%reg%" ADD %AU_KEY% /v UseWUServer /t REG_DWORD /d 1 /f || goto :exitWithError

echo. && echo set fake address for WindowsUpdate service -
%write% :WUServer = http://127.0.0.2: 
"%reg%" ADD %WU_KEY% /v WUServer /t REG_SZ /d "http://127.0.0.2" /f || goto :exitWithError

%write% :WUStatusServer = http://127.0.0.2: 
"%reg%" ADD %WU_KEY% /v WUStatusServer /t REG_SZ /d "http://127.0.0.2" /f || goto :exitWithError

%write% :UpdateServiceUrlAlternate = http://127.0.0.2: 
"%reg%" ADD %WU_KEY% /v UpdateServiceUrlAlternate /t REG_SZ /d "http://127.0.0.2" /f || goto :exitWithError

%write% :FillEmptyContentUrls = 1: 
"%reg%" ADD %WU_KEY% /v FillEmptyContentUrls /t REG_DWORD /d 1 /f || goto :exitWithError

echo. && echo telling WindowsUpdate never goto home
%write% :DoNotConnectToWindowsUpdateInternetLocations = 1: 
"%reg%" ADD %WU_KEY% /v DoNotConnectToWindowsUpdateInternetLocations /t REG_DWORD /d 1 /f || goto :exitWithError

echo. && echo never try to update device drivers via WindowsUpdate
%write% :ExcludeWUDriversInQualityUpdate = 1: 
"%reg%" ADD %WU_KEY% /v ExcludeWUDriversInQualityUpdate /t REG_DWORD /d 1 /f  || goto :exitWithError
